...
let barWidth = (circumference / (barCount * 2)) * 0.7;
...
function drawBiDirectionalBar(cx, cy, angle, radius, inLen, outLen, w, color) {
  let startX = cx + Math.cos(angle) * (radius - inLen);
  let startY = cy + Math.sin(angle) * (radius - inLen);
  let endX = cx + Math.cos(angle) * (radius + outLen);
  let endY = cy + Math.sin(angle) * (radius + outLen);
...
for (let i = 0; i < barCount; i++) {
  ...
  let angleRight = -Math.PI / 2 + (ratio * Math.PI);
  drawBiDirectionalBar(centerX, centerY, angleRight, baseRadius, inLength, outLength, barWidth, color);

  if (i > 0 && i < barCount) {
    let angleLeft = -Math.PI / 2 - (ratio * Math.PI);
    drawBiDirectionalBar(centerX, centerY, angleLeft, baseRadius, inLength, outLength, barWidth, color);
  }
}
